...
{
    public class HealingPotions : Potion
    {
        public override void Run()
        {
            ...
            Wait(2);
            // путь друида тоже закрыть
            if (client.inspectorData.Exist("MessageBoxFrame(Clone)"))
            {
                Logger.Instance.Send("увидели путь друида");
                client.WaitShowElementGUIAndMouseClick("CloseButtonIcon", 0, 10);
            }

            client.EnterCheat("/level 10", "/goldadd 1000000", "/emerald 100000");

            //идем покупать зелья!
            BuyPotion(client, 2, 0, 0);
            BuyPotion(client, 2, 1, 1);
            BuyPotion(client, 2, 2, 2);
            BuyPotion(client, 2, 3, 3);
            client.WaitShowElementGUIAndMouseClick("ButtonBack.Arrow", 1);

            client.GoToTestLevel();
            client.EnterCheat("/statadd 100000 HpAdd", "/statadd -150 HpRegenAdd", "/hp 1");

            //используем зелья и смотрим сколько хп они восстанавливают
            UsePotionAndEqualHp(client, 3, 0);
            client.EnterCheat("/hp 1");
            UsePotionAndEqualHp(client, 2, 1);
            client.EnterCheat("/hp 1");
            UsePotionAndEqualHp(client, 1, 2);
            client.EnterCheat("/hp 1");
            UsePotionAndEqualHp(client, 0, 3);
            ...
            Wait(() => client.inspectorData.Exist("ChangePersonButtonHolder.LobbyMenuButtonFrame(Clone)"), 10);
            client.EnterCheat("/getitem 111 3"); // чемпионское зелье хп
            Wait(1);
            ...
            Wait(1);
            client.WaitShowElementGUIAndMouseClick("ItemFramePW3(Clone)", 4, 3); // чемпионское зелье хп
            Wait(1);
            ...
            UsePotionAndEqualHp(client, 0, 4);
            ExitToLobby(client);
        }
